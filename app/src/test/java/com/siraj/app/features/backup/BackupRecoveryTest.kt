...
class BackupRecoveryTest {
    ...
    @Test
    fun `initial backup snapshots are loaded with valid CMEK and SHA256 signatures`() =
        runTest {
            ...
            assertTrue(
                "Storage location must use isolated vault bucket",
                prodSnapshot.storageLocationUri.contains("siraj-prod-backups-isolated-vault"),
            )
    ...
    @Test
    fun `triggering a new backup creates an encrypted snapshot with valid hash`() =
        runTest {
            val result =
    ...
    fun `right to be forgotten strictly excludes deleted user accounts during restoration`() {
        ...
        assertTrue(
            "Must not contain any records belonging to deleted user",
            sanitized.none {
                it["userId"] == "deleted_user_xyz" ||
                    it["authorId"] == "deleted_user_xyz"
            },
        )
    }

    @Test
    fun `dry-run restore executes successfully in isolated recovery sandbox`() =
        runTest {
            val result =
                repository.executeDryRunRestore(
    ...
    @Test
    fun `project level restore recovers specific project without restoring whole cluster`() =
        runTest {
            val result =
                repository.restoreProjectFromSnapshot(
                    snapshotId = "snap_prod_daily_01",
    ...
    @Test
    fun `viewModel filters snapshots and coordinates backup actions`() =
        runTest {
            advanceUntilIdle()
            ...
            assertTrue(
                "Filtered snapshots should only contain PROD",
                state.filteredSnapshots.all { it.environment == BackupEnvironment.PROD },
            )
