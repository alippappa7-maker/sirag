...
class SubscriptionViewModelTest {
    ...
    @Test
    fun `initialization loads user balances correctly`() = runTest {
        viewModel = SubscriptionViewModel(repository, billingManager)
        advanceUntilIdle()

        assertEquals(100, viewModel.state.value.balance?.availableCredits)
        assertEquals(50, viewModel.state.value.balance?.totalPurchased)
        assertEquals(0, viewModel.state.value.balance?.totalUsed)
    }

    @Test
    fun `initialization triggers billing connection`() = runTest {
        viewModel = SubscriptionViewModel(repository, billingManager)
        advanceUntilIdle()
