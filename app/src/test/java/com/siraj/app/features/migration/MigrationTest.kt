...
import com.siraj.app.domain.models.migration.SchemaConstants
...
class MigrationTest {
    ...
    @Test
    fun `test dry run migration does not alter actual data`() = runTest {
        // Create Dry Run Job
    ...
    @Test
    fun `test real migration alters data and backward compatibility is preserved`() = runTest {
         // Create Real Job
    ...
    @Test
    fun `test concurrent migration lock prevents multiple jobs`() = runTest {
        ...
        assertEquals("Could not acquire lock: Migration lock already held by another job: ${job1.jobId}", (finalJob2Res as Resource.Error).message)
    }
}
