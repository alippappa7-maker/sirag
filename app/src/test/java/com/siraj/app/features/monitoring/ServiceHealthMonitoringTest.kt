...
class ServiceHealthMonitoringTest {
    ...
    @Test
    fun `all 13 essential platform services are monitored and loaded with initial health status`() =
        runTest {
    ...
    fun `public user messages are politely sanitized hiding cloud cluster details`() {
        val geminiError =
            HealthMonitoringEngine.sanitizeForUser(
        ...
        assertFalse(
            "User message must never leak hostnames or API key pointers",
            geminiError.contains("us-central1") || geminiError.contains("Bearer"),
        )
        assertTrue(
            "Must provide polite Arabic fallback advice",
            geminiError.contains("مساعد الإنتاج الذكي") || geminiError.contains("صيانة مؤقتة"),
        )
    ...
    @Test
    fun `tripping circuit breaker disables service and routes to fallback gracefully`() =
        runTest {
            val result =
                repository.toggleServiceCircuitBreaker(
    ...
    @Test
    fun `creating an incident logs timeline events and updates service status`() =
        runTest {
            val incidentResult =
            ...
            val updateRes =
                repository.updateIncidentState(
    ...
    @Test
    fun `viewModel coordinates probes, category filters and incident lifecycle`() =
        runTest {
            advanceUntilIdle()
            ...
            assertTrue(
                "Filtered list should only contain AI providers",
                state.filteredServicesList.all {
                    it.service.category ==
                        ServiceCategory.AI_PROVIDERS
                },
            )
