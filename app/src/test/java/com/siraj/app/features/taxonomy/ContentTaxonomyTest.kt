...
class ContentTaxonomyTest {
    ...
    fun testQuranText_isStrictlyLockedAndImmutableAndImportedDataset() {
        val quranMeta = ContentTaxonomyEngine.validateAndEnforceTaxonomy(
        ...
        val quranItem = ClassifiedContentItem(
            id = "quran_1",
            title = "سورة الفاتحة",
            contentSnippet = "...",
            metadata = quranMeta
    ...
    fun testAiGenerated_neverEquatedToVerifiedEditorial() {
        val aiMeta = ContentTaxonomyEngine.validateAndEnforceTaxonomy(
    ...
    fun testLicensedExternal_neverOwnedBySiraj_andUnknownRightsRestricted() {
        ...
        val externalMeta = ContentTaxonomyEngine.validateAndEnforceTaxonomy(
        ...
        val unknownRightsMeta = ContentTaxonomyEngine.validateAndEnforceTaxonomy(
            originType = ContentOriginType.LICENSED_EXTERNAL,
            disciplineType = ContentDisciplineType.GENERAL,
    ...
    fun testUserGeneratedContent_neverAppearsAsOfficialEditorial() {
        val ugcMeta = ContentTaxonomyEngine.validateAndEnforceTaxonomy(
        ...
        val ugcItem = ClassifiedContentItem(
            id = "ugc_1",
            title = "مقال تجربة",
            contentSnippet = "...",
            metadata = ugcMeta
    ...
    fun testReviewPipelineRouting_forAllDisciplines() {
        val quranPath = ContentTaxonomyEngine.determineReviewPipeline(
            ContentOriginType.SYSTEM_CONTENT, ContentDisciplineType.QURAN_TEXT, GenerationMethod.IMPORTED_DATASET, TaxonomyVerificationStatus.SHARIA_VERIFIED
        )
        assertEquals(ReviewPipelinePath.LOCKED_IMMUTABLE_PASSTHROUGH, quranPath)

        val hadithPath = ContentTaxonomyEngine.determineReviewPipeline(
            ContentOriginType.EDITORIAL_CONTENT, ContentDisciplineType.HADITH, GenerationMethod.MANUAL_HUMAN, TaxonomyVerificationStatus.SHARIA_VERIFIED
        )
        assertEquals(ReviewPipelinePath.SHARIA_SCHOLAR_MANDATORY, hadithPath)

        val tafsirPath = ContentTaxonomyEngine.determineReviewPipeline(
            ContentOriginType.EDITORIAL_CONTENT, ContentDisciplineType.TAFSIR, GenerationMethod.MANUAL_HUMAN, TaxonomyVerificationStatus.SHARIA_VERIFIED
        )
        assertEquals(ReviewPipelinePath.SHARIA_SCHOLAR_MANDATORY, tafsirPath)

        val editorialPath = ContentTaxonomyEngine.determineReviewPipeline(
            ContentOriginType.EDITORIAL_CONTENT, ContentDisciplineType.GENERAL, GenerationMethod.MANUAL_HUMAN, TaxonomyVerificationStatus.EDITORIAL_VERIFIED
        )
        assertEquals(ReviewPipelinePath.EDITORIAL_STANDARD, editorialPath)

        val externalPath = ContentTaxonomyEngine.determineReviewPipeline(
            ContentOriginType.LICENSED_EXTERNAL, ContentDisciplineType.GENERAL, GenerationMethod.MANUAL_HUMAN, TaxonomyVerificationStatus.UNVERIFIED
        )
    ...
    fun testTaxonomyAuditReport_calculatesAccurately() {
        val items = listOf(
            ClassifiedContentItem(
                ...
                title = "قرآن",
                contentSnippet = "...",
                metadata = ContentTaxonomyEngine.validateAndEnforceTaxonomy(
                    originType = ContentOriginType.SYSTEM_CONTENT,
                    disciplineType = ContentDisciplineType.QURAN_TEXT,
                    mediaType = ContentMediaType.TEXT,
    ...
    fun testLegacyMigration_migratesLegacyCategoriesSuccessfully() {
        val legacyList = listOf(
            ...
            LegacyContentItem("leg_2", "تفسير سورة الفلق", "tafsir", "تفسير الطبري", isQuran = false, isAi = false, ownerId = "editor_1"),
    ...
    @Test
    fun testRepositoryIntegration_andFilterOperations() = runTest {
        ...
        val updateRes = repo.updateTaxonomyMetadata(
            id = tafsirItem.id,
