...
class DefectManagementUnitTest {
    ...
    @Test
    fun `initial defects list contains all 8 required classifications and default items`() =
        runTest {
    ...
    @Test
    fun `triage summary accurately calculates counts across classifications and domains`() =
        runTest {
    ...
    @Test
    fun `sharia domain defect cannot be downgraded to minor or enhancement`() =
        runTest {
    ...
    @Test
    fun `closing a defect without a valid closure reason fails`() =
        runTest {
            val result =
    ...
    @Test
    fun `closing a defect with a valid closure reason succeeds`() =
        runTest {
            val result =
                repository.updateDefectStatus(
                    id = "BUG-004",
    ...
    @Test
    fun `resolving critical defect without resolution note fails`() =
        runTest {
            val result =
                repository.updateDefectStatus(
            ...
            assertEquals(
                "الأعطال الحرجة والمانعة للإطلاق (Blocker / Critical) تتطلب توثيق تفاصيل الحل الفني بدقة.",
                result.exceptionOrNull()?.message,
            )
        }

    @Test
    fun `resolving critical defect with detailed resolution note and verification test succeeds`() =
        runTest {
            val result =
                repository.updateDefectStatus(
                    id = "BUG-001",
    ...
    @Test
    fun `deferring a defect requires a valid reason`() =
        runTest {
            val resultFail =
                repository.updateDefectStatus(
                    id = "BUG-005",
    ...
    @Test
    fun `prioritized fix list places blockers and criticals ahead of minor enhancements and excludes closed items`() =
        runTest {
            ...
            assertTrue(
                "First item should be P0 or P1",
                firstItem.priority == DefectPriority.P0_IMMEDIATE || firstItem.priority == DefectPriority.P1_HIGH,
            )
            assertTrue(
                "First item should not be closed or deferred",
                firstItem.status != DefectStatus.CLOSED && firstItem.status != DefectStatus.DEFERRED,
            )
    ...
    @Test
    fun `safe logs do not contain raw pii or user sensitive tokens`() =
        runTest {
            val defects = repository.getAllDefects().first()
    ...
    @Test
    fun `registering new defect validates fields and assigns to repository`() =
        runTest {
            val newDefect =
                BetaDefectRecord(
                    id = "BUG-009",
