...
class IncidentResponseTest {
    ...
    @Test
    fun `emergency kill-switch halts publishing globally and logs immutable action`() = runTest {
        val res = repository.toggleGlobalPublishingHalt(
            halt = true,
    ...
    @Test
    fun `emergency secret rotation rejects unauthorized roles and approves security officer`() = runTest {
        // Unauthorized role attempt
        val failResult = IncidentResponseEngine.createEmergencyAction(
        ...
        val passResult = repository.rotateSecretKey(
    ...
    @Test
    fun `sharia correction requires verified sources and double reviewer approval for sacred content`() = runTest {
        // Incomplete correction without reviewer 2
        val singleRevCorrection = ShariaIncidentCorrection(
        ...
        val doubleApprovedCorrection = singleRevCorrection.copy(
            reviewer2Id = "REV-02",
    ...
    fun `public incident communication sanitizes technical stack details and protects secrets`() {
        val sanitizedOutage = IncidentResponseEngine.sanitizePublicIncidentNotice(
        ...
        val sanitizedSecretLeak = IncidentResponseEngine.sanitizePublicIncidentNotice(
    ...
    @Test
    fun `batch refund executes for duplicate charges and logs credit restitution`() = runTest {
        val refundRes = repository.executeBatchRefund(
    ...
    @Test
    fun `post-mortem report records complete root cause, downtime and preventive tasks`() = runTest {
    ...
    @Test
    fun `escalation roster has 6 24x7 emergency contacts with distinct roles`() = runTest {
    ...
    @Test
    fun `viewModel handles playbooks navigation and emergency actions`() = runTest {
        advanceUntilIdle()
