...
class AdminSecurityEngine {
    ...
    private fun hasPermissionForOperation(role: AdminRole, operation: SensitiveOperationType): Boolean {
        return when (role) {
    ...
    fun validateRoleAssignment(executorRole: AdminRole, targetNewRole: AdminRole): Boolean {
    ...
    fun requiresReAuthentication(operation: SensitiveOperationType, isDeviceTrusted: Boolean): Boolean {
        ...
        return when (operation) {
            ...
            SensitiveOperationType.DELETE_CONTENT -> true
            else -> false
        }
    }
}
