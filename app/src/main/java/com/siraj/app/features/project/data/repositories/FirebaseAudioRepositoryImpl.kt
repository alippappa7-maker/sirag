...
class FirebaseAudioRepositoryImpl(
    ...
    override suspend fun generateVoiceover(request: GenerateAudioRequest): Result<AudioItem> {
        return try {
            ...
            if (textLower.contains("بسم الله الرحمن الرحيم") &&
                (textLower.contains("قل هو الله") || textLower.contains("الحمد لله رب العالمين"))
            ) {
            ...
            val id =
                (audioData["id"] as? String) ?: java.util.UUID
                    .randomUUID()
                    .toString()
            ...
            firestore
                .collection("projects")
                .document(request.projectId)
                .collection("audios")
                .document(audioItem.id)
                .set(audioItem)
                .await()
    ...
    override suspend fun uploadUserAudio(
        ...
        return try {
            ...
            if (!allowedMime.contains(mimeType.lowercase()) &&
        ...
        return try {
            ...
            if (!currentAudio.sceneId.isNullOrBlank()) {
                val sceneDoc =
                    firestore
                        .collection("projects")
                        .document(currentAudio.projectId)
                        .collection("scenes")
                        .document(currentAudio.sceneId)
                        .get()
                        .await()
                if (sceneDoc.exists()) {
                    firestore
                        ...
                        .collection("scenes")
                        .document(currentAudio.sceneId)
                        .update("durationMs", newTrimmedDuration)
                        .await()
                }
            }
    ...
    override suspend fun getProjectAudios(projectId: String): Flow<List<AudioItem>> =
        callbackFlow {
            val listener =
                firestore
                    .collection("projects")
