...
class FirebaseRightsRepositoryImpl(
    ...
    override suspend fun updateAssetRights(assetId: String, updates: Map<String, Any?>): Resource<Unit> {
        return try {
    ...
    override fun getAssetsWithExpiringRights(workspaceId: String, daysThreshold: Int): Flow<Resource<List<Asset>>> = flow<Resource<List<Asset>>> {
        emit(Resource.Loading)
    ...
    override suspend fun verifyRightsForExport(projectAssetIds: List<String>): Resource<Boolean> {
        return try {
            ...
            val snapshot = firestore.collection("assets")
                ...
                .await()
            val assets = snapshot.documents.mapNotNull { it.toObject(Asset::class.java) }
            val hasUnknownRights = assets.any { it.rightsStatus == RightsStatus.UNKNOWN || it.rightsStatus == RightsStatus.REJECTED || it.rightsStatus == RightsStatus.EXPIRED }
            if (hasUnknownRights) {
