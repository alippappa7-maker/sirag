...
enum class ReviewerTabFilter(val arabicTitle: String) {
    ALL("الكل"),
...
class ReviewerGovernanceViewModel(
            ...
            repository.getReviewers().collect { res ->
                if (res is Resource.Success) {
                    ...
                    _uiState.update { current ->
                        current.copy(
                            reviewers = list,
                            filteredReviewers = filterReviewers(list, current.selectedTab, current.selectedDomainFilter, current.searchQuery),
                            isLoading = false
                        )
                    }
    ...
    fun createNewReviewer(
        ...
        viewModelScope.launch {
            val newProfile = ReviewerProfile(
    ...
    fun verifyReviewer(reviewerId: String, reverificationDurationDays: Long = 365) {
        viewModelScope.launch {
            val dueTimestamp = System.currentTimeMillis() + (reverificationDurationDays * 24 * 3600 * 1000L)
            val result = repository.verifyReviewerByOwner(
    ...
    fun suspendReviewer(reviewerId: String, reason: String) {
        viewModelScope.launch {
            val result = repository.suspendReviewer(
                reviewerId = reviewerId,
                ownerId = _uiState.value.currentOwnerId,
                reason = reason
            )
    ...
    fun reactivateReviewer(reviewerId: String) {
        viewModelScope.launch {
            val result = repository.reactivateReviewer(
                reviewerId = reviewerId,
                ownerId = _uiState.value.currentOwnerId
            )
            when (result) {
    ...
    fun addQualification(
        ...
        viewModelScope.launch {
            val qual = ReviewerQualification(
    ...
    fun calculateAssignmentProposal(item: ShariaReviewItem, domain: ReviewerDomain) {
        val proposal = ReviewerGovernanceEngine.proposeAssignment(
            item = item,
            domain = domain,
    ...
    fun assignReviewersToItem(
        ...
        viewModelScope.launch {
            ...
            val primaryEligibility = ReviewerGovernanceEngine.validateReviewerEligibility(
                primaryReviewer, domain, item.riskLevel, criticalTopic, isSecondReviewer = false
            )
            ...
            val primaryConflict = ReviewerGovernanceEngine.checkConflictOfInterest(
                primaryReviewer.id, item.creatorId, item.projectId, _uiState.value.conflicts
            )
            ...
            if (secondReviewer != null) {
                ...
                val secondEligibility = ReviewerGovernanceEngine.validateReviewerEligibility(
                    secondReviewer, domain, item.riskLevel, criticalTopic, isSecondReviewer = true
                )
                ...
                val secondConflict = ReviewerGovernanceEngine.checkConflictOfInterest(
                    secondReviewer.id, item.creatorId, item.projectId, _uiState.value.conflicts
                )
            ...
            val assignment = ReviewerAssignment(
                itemId = item.id,
    ...
    fun submitImmutableDecision(
        ...
        viewModelScope.launch {
            ...
            val decision = ReviewerGovernanceEngine.createImmutableDecision(
