...
enum class AudioSourceType(val displayName: String, val isAiGenerated: Boolean) {
...
enum class AudioVoiceGender(val displayName: String) {
    MALE("صوت رجالي"),
    FEMALE("صوت نسائي")
}
...
enum class AudioLanguage(val code: String, val displayName: String) {
...
sealed class AudioGenerationUiState {
    ...
    data class Generating(val progressMessage: String) : AudioGenerationUiState()
    data class Success(val audioItem: AudioItem) : AudioGenerationUiState()
    data class Error(val message: String) : AudioGenerationUiState()
}

sealed class AudioUploadUiState {
    object Idle : AudioUploadUiState()
    data class Uploading(val progress: Float) : AudioUploadUiState()
    data class Success(val audioItem: AudioItem) : AudioUploadUiState()
