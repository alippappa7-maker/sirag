                        ...
                        snapshot
                            ...
                            ?.mapNotNull { doc ->
                                try {
                                    ...
                                    val sourceType =
                                        try {
                                            SubtitleSourceType.valueOf(srcTypeStr)
                                        } catch (
                                            _: Exception,
                                        ) {
                                            SubtitleSourceType.SCENE_NARRATION
                                        }
                                    val reviewStr = doc.getString("reviewStatus") ?: SubtitleReviewStatus.NOT_REQUIRED.name
                                    val reviewStatus =
                                        try {
                                            SubtitleReviewStatus.valueOf(reviewStr)
                                        } catch (
                                            _: Exception,
                                        ) {
                                            SubtitleReviewStatus.NOT_REQUIRED
                                        }
                                    ...
                                    val style =
                                        if (styleMap != null) {
                                            ...
                                            val fontFamily =
                                                try {
                                                    SubtitleFontFamily.valueOf(fontStr)
                                                } catch (_: Exception) {
                                                    SubtitleFontFamily.SYSTEM_SANS
                                                }
                                            val posStr = styleMap["position"] as? String ?: SubtitlePosition.BOTTOM.name
                                            val position =
                                                try {
                                                    SubtitlePosition.valueOf(posStr)
                                                } catch (_: Exception) {
    ...
    override suspend fun autoTranslateToEnglish(
        ...
        try {
            ...
            for (sub in arabicSubtitles) {
                val isSacred =
                    sub.locked ||
                        sub.sourceType == SubtitleSourceType.QURAN_SOURCE_LOCKED ||
                        sub.sourceType == SubtitleSourceType.HADITH_SOURCE_LOCKED

                // Construct honest, review-required English translation draft
                val englishTextDraft =
                    if (isSacred) {
