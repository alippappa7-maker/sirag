...
class NotificationViewModel(
    ...
    private fun observePreferences() {
        viewModelScope.launch {
            repository.getPreferencesFlow(currentUserId)
                .collect { prefs ->
    ...
    private fun applyFilter(list: List<SirajNotification>, filter: NotificationFilter): List<SirajNotification> {
        return when (filter) {
            NotificationFilter.ALL -> list
            NotificationFilter.UNREAD -> list.filter { !it.isRead }
            NotificationFilter.PROJECTS -> list.filter {
    ...
    fun sendTestNotification(
        ...
        viewModelScope.launch {
            val title = customTitle?.ifBlank { null } ?: when (type) {
            ...
            val body = customBody?.ifBlank { null } ?: when (type) {
            ...
            val testNotif = SirajNotification(
                id = "test_${System.currentTimeMillis()}",
                userId = currentUserId,
                type = type,
                title = title,
                body = body,
                entityType = entityType ?: when (type) {
