...
class FirebaseVideoCompositionServiceImpl(
    ...
    override suspend fun buildManifest(
        ...
        return try {
            ...
            val compositionScenes = scenes.sortedBy { it.orderIndex }.map { scene ->
                val visualType = when (scene.backgroundType) {
                ...
                val transition = when (scene.transition) {
            ...
            val totalDurationMs = if (isPreview) {
            ...
            // 3. Fetch Subtitles
            val subtitlesSnapshot = firestore.collection("projects").document(projectId)
                .collection("subtitles")
                .orderBy("startMs")
                .get()
                .await()

            val compositionSubtitles = subtitlesSnapshot.documents.mapNotNull { doc ->
            ...
            val audioSnapshot = firestore.collection("projects").document(projectId)
                .collection("audio")
                .get()
                .await()
            val audioItems = audioSnapshot.documents.mapNotNull { it.toObject(AudioItem::class.java) }
            val primaryVoiceover = audioItems.firstOrNull { it.sourceType == AudioSourceType.GENERATED_VOICE || it.sourceType == AudioSourceType.QURAN_RECITATION }

            val audioMix = CompositionAudioTrack(
                voiceoverUrl = primaryVoiceover?.audioUrl,
                recitationUrl = if (primaryVoiceover?.sourceType == AudioSourceType.QURAN_RECITATION) primaryVoiceover.audioUrl else null,
                voiceVolume = 1.0f,
                soundtrackUrl = null,
            ...
            val sourceCitation = if (isApproved && project.description.isNotBlank()) {
                "محتوى معتمد وموثق - منصة سراج"
            } else null

            val branding = CompositionBranding(
                isIslamicVerified = isApproved,
                sourceCitationText = sourceCitation,
                showWatermark = includeWatermark,
                attributionCredits = if (includeWatermark) listOf("تم الإنتاج بواسطة منصة سراج للإنتاج الإسلامي الموثق") else emptyList()
            )
            ...
            val manifest = VideoCompositionManifest(
    ...
    override fun executeComposition(
        ...
        try {
            ...
            currentJob = currentJob.copy(
                ...
                progress = 45,
                logs = currentJob.logs + "تركيب المشاهد: دمج طبقات الفيديو والصور والمؤثرات الانتقالية مع مسارات الصوت والترجمة$isVerifiedText."
            )
            updateJobInFirestore(currentJob)
            emit(currentJob)
            ...
            currentJob = currentJob.copy(
                ...
                progress = 75,
                logs = currentJob.logs + "ترميز الفيديو: تصيير نهائي بمقاس ${manifest.aspectRatio} وجودة ${manifest.quality.label} (H.264/AAC)$subText."
            )
            updateJobInFirestore(currentJob)
            emit(currentJob)
            ...
            val estimatedSizeBytes = (manifest.totalDurationMs / 1000) * when (manifest.quality) {
            ...
            val secureThumbnailUrl = manifest.scenes.firstOrNull()?.visualUrl?.ifBlank { null }
            ...
            firestore.collection("projects").document(manifest.projectId).update(
                mapOf(
