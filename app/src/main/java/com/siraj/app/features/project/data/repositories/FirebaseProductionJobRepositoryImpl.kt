...
class FirebaseProductionJobRepositoryImpl(
    ...
    override suspend fun createJob(
        ...
        return try {
            ...
            val calculatedUnits =
                (baseUnits * quality.costMultiplier * fpsMultiplier * (if (isPreviewOnly) 0.4 else 1.0))
                    .toLong()
                    .coerceAtLeast(
                        2L,
                    )
    ...
    override fun observeJob(jobId: String): Flow<ProductionJob?> =
        callbackFlow {
            val listener =
                firestore
                    .collection("production_jobs")
    ...
    override fun getJobsForProject(projectId: String): Flow<List<ProductionJob>> =
        callbackFlow {
            val listener =
                firestore
                    .collection("production_jobs")
    ...
    override fun getJobsForUser(userId: String): Flow<List<ProductionJob>> =
        callbackFlow {
            val listener =
                firestore
                    .collection("production_jobs")
