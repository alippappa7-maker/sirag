...
fun AppearanceSettings(
    ...
    Column(modifier = Modifier.padding(16.dp)) {
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            RadioButton(selected = prefs.themeMode == ThemeMode.LIGHT, onClick = {
                viewModel.updatePreferences { it.copy(themeMode = ThemeMode.LIGHT) }
            })
            Text("فاتح")
            Spacer(modifier = Modifier.width(16.dp))
            RadioButton(
                selected = prefs.themeMode == ThemeMode.DARK,
                onClick = { viewModel.updatePreferences { it.copy(themeMode = ThemeMode.DARK) } },
            )
            Text("داكن")
            Spacer(modifier = Modifier.width(16.dp))
            RadioButton(selected = prefs.themeMode == ThemeMode.SYSTEM, onClick = {
                viewModel.updatePreferences { it.copy(themeMode = ThemeMode.SYSTEM) }
            })
...
fun LanguageSettings(
    ...
    Column(modifier = Modifier.padding(16.dp)) {
        Text(
            androidx.compose.ui.res
                .stringResource(com.siraj.app.R.string.language),
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            ...
            Text(
                androidx.compose.ui.res
                    .stringResource(com.siraj.app.R.string.language_arabic),
            )
            Spacer(modifier = Modifier.width(16.dp))
            ...
            Text(
                androidx.compose.ui.res
                    .stringResource(com.siraj.app.R.string.language_english),
            )
        }
        Spacer(modifier = Modifier.height(24.dp))
        OutlinedTextField(
    ...
    LazyColumn(modifier = Modifier.padding(16.dp)) {
        item {
            ...
            CalculationMethod.values().forEach { method ->
                Row(verticalAlignment = Alignment.CenterVertically) {
                    RadioButton(selected = prefs.calculationMethod == method, onClick = {
                        viewModel.updatePreferences { p ->
                            p.copy(calculationMethod = method)
                        }
                    })
                    Text(method.name)
                }
            ...
            Madhab.values().forEach { madhab ->
                Row(verticalAlignment = Alignment.CenterVertically) {
                    RadioButton(selected = prefs.madhab == madhab, onClick = {
                        viewModel.updatePreferences { p ->
                            p.copy(madhab = madhab)
                        }
                    })
                    Text(madhab.name)
                }
...
fun VideoSettings(
    ...
    Column(modifier = Modifier.padding(16.dp)) {
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            RadioButton(selected = prefs.videoQuality == VideoQuality.HIGH, onClick = {
                viewModel.updatePreferences { p ->
                    p.copy(videoQuality = VideoQuality.HIGH)
                }
            })
            Text("عالية")
            Spacer(modifier = Modifier.width(16.dp))
            RadioButton(selected = prefs.videoQuality == VideoQuality.MEDIUM, onClick = {
                viewModel.updatePreferences { p ->
                    p.copy(videoQuality = VideoQuality.MEDIUM)
                }
            })
            Text("متوسطة")
            Spacer(modifier = Modifier.width(16.dp))
            RadioButton(selected = prefs.videoQuality == VideoQuality.LOW, onClick = {
                viewModel.updatePreferences { p ->
                    p.copy(videoQuality = VideoQuality.LOW)
                }
            })
        ...
        Row(
            ...
            Switch(checked = prefs.downloadWifiOnly, onCheckedChange = {
                viewModel.updatePreferences { p ->
                    p.copy(downloadWifiOnly = it)
                }
            })
        }
    }
...
fun PrivacySettings(
    ...
    Column(modifier = Modifier.padding(16.dp)) {
        ...
        Text(
            "يمكن تفعيل البصمة أو الوجه في حال تم تفعيل القفل.",
        ...
        Spacer(modifier = Modifier.height(8.dp))
        Text(
            "نجمع بيانات استخدام مجهولة (بدون IP أو نصوص شخصية) لتحسين جودة التطبيق فقط.",
            style = MaterialTheme.typography.bodySmall,
            color = MaterialTheme.colorScheme.onSurfaceVariant,
        )
        ...
        Spacer(modifier = Modifier.height(8.dp))
        Text(
            "إرسال تقارير الأعطال الفنية تلقائياً للمساعدة في حل المشكلات التقنية مع التزام تام بعدم جمع أي نصوص قرآنية أو أحاديث أو محتوى خاص أو أسرار.",
            style = MaterialTheme.typography.bodySmall,
            color = MaterialTheme.colorScheme.onSurfaceVariant,
        )
        ...
        Spacer(modifier = Modifier.height(8.dp))
        Text(
            "التحكم في حفظ مواضع المشاهدة والاستماع وسياسة الاحتفاظ بالبيانات.",
            style = MaterialTheme.typography.bodySmall,
            color = MaterialTheme.colorScheme.onSurfaceVariant,
        )
...
fun IslamicSettings(
    ...
    Column(modifier = Modifier.padding(16.dp)) {
        ...
        Spacer(modifier = Modifier.height(8.dp))
        Text(
            "تطبيق سراج يعتمد على مراجعة بشرية للقرارات الحساسة ويطبق سياسة استخدام واضحة لضمان سلامة وموثوقية المحتوى.",
            style = MaterialTheme.typography.bodySmall,
            color = MaterialTheme.colorScheme.onSurfaceVariant,
        )
        Spacer(modifier = Modifier.height(16.dp))
...
fun StorageSettings(
    ...
    Column(modifier = Modifier.padding(16.dp)) {
        ...
        Spacer(modifier = Modifier.height(8.dp))
        Text(
            "التحكم في المساحة التخزينية المحلية وتصدير أو حذف البيانات بأمان.",
            style = MaterialTheme.typography.bodySmall,
            color = MaterialTheme.colorScheme.onSurfaceVariant,
        )
        Spacer(modifier = Modifier.height(16.dp))
                ...
                confirmButton = {
                    Button(
                        onClick = {
                            ...
                            com.siraj.app.core.privacy.PrivacyManager
    ...
    Column(modifier = Modifier.padding(16.dp)) {
        ...
        SirajTechCard(
            ...
            Column(modifier = Modifier.padding(12.dp)) {
                Text(
                    "البيئة: ${com.siraj.app.core.config.EnvironmentConfig.currentEnvironment.displayName}",
                    style = MaterialTheme.typography.bodySmall,
                )
                Text(
                    "حالة المراقبة: ${if (com.siraj.app.core.monitoring.CrashMonitoringManager
                            .isCollectionEnabled()
                    ) {
                        "مفعلة (Active)"
                    } else {
                ...
                Text(
                    "نسخة التطبيق: ${com.siraj.app.core.config.EnvironmentConfig.buildIdentifier}",
                    style = MaterialTheme.typography.bodySmall,
                )
            }
        }
        ...
        OutlinedButton(
            onClick = {
                com.siraj.app.core.monitoring.CrashMonitoringManager
                    .triggerTestNonFatalError("طلب فحص يدوي من لوحة الإعدادات")
        ...
        if (showCrashDialog) {
            AlertDialog(
                ...
                text = {
                    Text(
                        "هذا الإجراء سيقوم بإحداث استثناء برمجي مقصود لإغلاق التطبيق فجأة لاختبار وصول تقرير Crashlytics إلى لوحة التحكم. هل تود المتابعة؟",
                    )
                },
                confirmButton = {
                    Button(
...
fun AboutSettings(
    ...
    Column(modifier = Modifier.padding(16.dp)) {
        Row(verticalAlignment = Alignment.CenterVertically) {
            Text(
                com.siraj.app.core.config.EnvironmentConfig.releaseLabel,
        ...
        SirajTechCard(
            ...
            Column(modifier = Modifier.padding(14.dp)) {
                ...
                Text(
                    "رقم البناء (Build Code): ${com.siraj.app.core.config.EnvironmentConfig.versionCode}",
                    style = MaterialTheme.typography.bodySmall,
                )
                Text(
                    "البيئة التشغيلية: ${com.siraj.app.core.config.EnvironmentConfig.currentEnvironment.displayName}",
                    style = MaterialTheme.typography.bodySmall,
                )
                Text(
                    "المعرف الشامل: ${com.siraj.app.core.config.EnvironmentConfig.buildIdentifier}",
                    style = MaterialTheme.typography.bodySmall,
                )
            }
        }
