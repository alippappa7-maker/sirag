...
object IncidentResponseEngine {
    ...
    val STANDARD_CONTACTS_MATRIX = listOf(
        IncidentContact(
            contactId = "CONT-001",
    ...
    fun validateShariaCorrection(
        ...
        val isDoubleApproved = !correction.reviewer2Id.isNullOrBlank() && 
                correction.reviewer2NotesArabic?.isNotBlank() == true

        val verifiedCorrection = correction.copy(
     ...
     */
    fun generateIncidentFingerprint(incidentType: IncidentType, sourceId: String): String {
    ...
    fun createEmergencyAction(
        ...
        val isAuthorized = when (actionType) {
        ...
        if (!isAuthorized) {
            return Result.failure(SecurityException("الدور [${executedByRole.displayNameArabic}] غير مخول بتنفيذ الإجراء الطارئ [${actionType.displayNameArabic}]"))
        }

        val record = EmergencyActionRecord(
