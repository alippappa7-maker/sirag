...
sealed class Screen(
    ...
    object ShareRouter : Screen("share_router/{linkId}?token={token}") {
        ...
        fun createRoute(
            linkId: String,
            token: String? = null,
        ): String = if (token != null) "share_router/$linkId?token=$token" else "share_router/$linkId"
    }
    ...
    object SceneEditor : Screen("scene_editor/{projectId}/{sceneId}") {
        fun createRoute(
            projectId: String,
            sceneId: String,
        ) = "scene_editor/$projectId/$sceneId"
    }
    ...
    object ScriptEditor : Screen("script_editor/{projectId}/{sceneId}?initialText={initialText}") {
        fun createRoute(
            ...
            "script_editor/$projectId/$sceneId?initialText=${java.net.URLEncoder.encode(initialText, "UTF-8")}"
        } else if (sceneId != null) {
            "script_editor/$projectId/$sceneId"
        } else {
            "script_editor/$projectId"
        }
    }
    ...
    object ImageStudio : Screen("image_studio/{projectId}/{sceneId}?initialPrompt={initialPrompt}") {
        fun createRoute(
            ...
            "image_studio/$projectId/$sceneId?initialPrompt=${java.net.URLEncoder.encode(initialPrompt, "UTF-8")}"
        } else if (sceneId != null) {
            "image_studio/$projectId/$sceneId"
        } else {
            "image_studio/$projectId"
        }
    }
    ...
    object AudioStudio : Screen("audio_studio/{projectId}/{sceneId}?initialText={initialText}") {
        fun createRoute(
            ...
            "audio_studio/$projectId/$sceneId?initialText=${java.net.URLEncoder.encode(initialText, "UTF-8")}"
        } else if (sceneId != null) {
            "audio_studio/$projectId/$sceneId"
        } else {
            "audio_studio/$projectId"
        }
    }
    ...
    object SoundtrackLibrary : Screen("soundtrack_library/{projectId}?sceneId={sceneId}") {
        fun createRoute(
            ...
            "soundtrack_library/$projectId?sceneId=$sceneId"
        } else {
            "soundtrack_library/$projectId"
        }
    }
    ...
    object SubtitleEditor : Screen("subtitle_editor/{projectId}/{sceneId}?initialText={initialText}") {
        fun createRoute(
            ...
            "subtitle_editor/$projectId/$sceneId?initialText=${java.net.URLEncoder.encode(initialText, "UTF-8")}"
        } else if (sceneId != null) {
            "subtitle_editor/$projectId/$sceneId"
        } else {
            "subtitle_editor/$projectId"
        }
    }
    ...
    object AiImageGenerator : Screen("ai_image_generator/{projectId}?sceneId={sceneId}") {
        fun createRoute(
            ...
            "ai_image_generator/$projectId?sceneId=$sceneId"
        } else {
            "ai_image_generator/$projectId"
        }
    }
    ...
    object Surah : Screen("surah/{surahId}?surahName={surahName}") {
        fun createRoute(
            id: Int,
            name: String,
        ) = "surah/$id?surahName=${java.net.URLEncoder.encode(name, "UTF-8")}"
    }
    ...
    object CreateTicket : Screen("create_ticket?category={category}") {
        fun createRoute(category: String? = null) = if (category != null) "create_ticket?category=$category" else "create_ticket"
    }
