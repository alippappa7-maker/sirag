...
class FirebaseSupportRepositoryImpl : SupportRepository {
    ...
    override fun getArticlesStream(query: String?, category: HelpCategory?): Flow<List<HelpArticle>> {
        return articlesState.asStateFlow().map { list ->
            list.filter { article ->
    ...
    override fun getUserTicketsStream(userId: String): Flow<List<SupportTicket>> {
        return ticketsState.asStateFlow().map { list ->
            list.filter { it.userId == userId || userId == "admin_or_reviewer" }
    ...
    override suspend fun createTicket(ticket: SupportTicket): Result<SupportTicket> {
        ...
        val finalTicketNumber = if (ticket.ticketNumber.isBlank()) {
            SupportSanitizerEngine.generateTicketNumber()
        } else {
            ticket.ticketNumber
        }

        val initialReply = TicketReply(
            id = UUID.randomUUID().toString(),
            ticketId = ticket.id,
            authorId = "system",
            authorName = "نظام سراج الآلي",
            authorRole = ReplyAuthorRole.SYSTEM,
            message = "مرحباً بك. تم استلام تذكرتك وتوجيهها آلياً إلى [${targetTeam.titleAr}]. سنقوم بالرد عليك في أقرب وقت ممكن.\n" +
                    SupportSanitizerEngine.SHARIA_DISCLAIMER_TEXT
        )

        val fullTicket = ticket.copy(
            ticketNumber = finalTicketNumber,
            targetTeam = targetTeam,
            isShariaReport = ticket.category == TicketCategory.SHARIA_CONTENT_ERROR || ticket.category == TicketCategory.MIHRAB_AND_QURAN,
    ...
    override suspend fun addReply(ticketId: String, reply: TicketReply): Result<Unit> {
        val currentList = ticketsState.value.toMutableList()
        ...
        if (index != -1) {
            ...
            val newStatus = if (reply.authorRole == ReplyAuthorRole.USER && oldTicket.status == TicketStatus.WAITING_USER) {
    ...
    override suspend fun updateTicketStatus(ticketId: String, status: TicketStatus, note: String?): Result<Unit> {
        val currentList = ticketsState.value.toMutableList()
        val index = currentList.indexOfFirst { it.id == ticketId }
        if (index != -1) {
            val oldTicket = currentList[index]
            val now = System.currentTimeMillis()
            val extraReplies = if (!note.isNullOrBlank()) {
                oldTicket.replies + TicketReply(
                    id = UUID.randomUUID().toString(),
                    ticketId = ticketId,
                    authorId = "system",
                    authorName = "النظام",
    ...
    override suspend fun rateTicket(ticketId: String, stars: Int, feedback: String?): Result<Unit> {
