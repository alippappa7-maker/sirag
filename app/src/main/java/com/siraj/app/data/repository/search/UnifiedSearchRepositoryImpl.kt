...
class UnifiedSearchRepositoryImpl(
    ...
    private val verifiedSourcesCatalog = listOf(
        SearchResultItem(
            id = "src_bukhari",
            title = "صحيح البخاري",
    ...
    private val verifiedFlashesCatalog = listOf(
        SearchResultItem(
            id = "flash_1",
    ...
    override suspend fun search(
        ...
        userId: String?,
        workspaceId: String?
    ): Resource<GlobalSearchResult> = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            if (!filter.onlyPrivateProjects && (filter.category == SearchCategory.ALL || filter.category == SearchCategory.PROJECT) ||
            ...
            SearchCategory.values().forEach { cat ->
                categoryCounts[cat] = if (cat == SearchCategory.ALL) {
            ...
            var filteredItems = allMatchedItems.filter { item ->
                // Category Filter
                val matchesCategory = (filter.category == SearchCategory.ALL) || (item.category == filter.category)

                // Language Filter
                val matchesLanguage = when (filter.language) {
                    SearchLanguage.ALL -> true
                    SearchLanguage.ARABIC -> item.language.contains("عرب", ignoreCase = true) || item.language.equals("ar", ignoreCase = true)
                    SearchLanguage.ENGLISH -> item.language.contains("انجليز", ignoreCase = true) || item.language.equals("en", ignoreCase = true)
                    SearchLanguage.URDU -> item.language.contains("أرد", ignoreCase = true) || item.language.equals("ur", ignoreCase = true)
                    SearchLanguage.FRENCH -> item.language.contains("فرنس", ignoreCase = true) || item.language.equals("fr", ignoreCase = true)
                    SearchLanguage.OTHER -> true
                }

                // Content Type Filter
                val matchesContentType = when (filter.contentType) {
                    ...
                    SearchContentType.VIDEO -> item.category == SearchCategory.PROJECT || item.category == SearchCategory.TEMPLATE
                ...
                val matchesVerification = when (filter.verificationFilter) {
            ...
            // Apply Sorting
            filteredItems = when (filter.sortOption) {
    ...
    private suspend fun searchAudio(query: String): List<SearchResultItem> {
        ...
        val audioRes = audioRepository.getTracks(
    ...
    private fun searchFlashes(query: String): List<SearchResultItem> {
        return verifiedFlashesCatalog.filter { flash ->
            query.isBlank() ||
                ...
                if (query.isBlank() ||
                    ...
                    results.add(
                        SearchResultItem(
                            ...
                            snippet = template.description.ifBlank { "قالب إنتاج فيديو لمنصة ${template.recommendedPlatform} بأسلوب ${template.sceneStyle}." },
    ...
    private fun searchSources(query: String): List<SearchResultItem> {
        return verifiedSourcesCatalog.filter { src ->
            query.isBlank() ||
    ...
    private suspend fun searchPrivateProjects(
        ...
        val projectsRes = projectRepository.getProjects(
            workspaceId = wsId,
            limit = 30,
            offset = 0,
            query = query
    ...
    override suspend fun getSuggestions(query: String, limit: Int): List<SearchSuggestion> = withContext(Dispatchers.IO) {
    ...
    override fun getSearchHistory(userId: String?): Flow<List<SearchHistoryItem>> {
        return historyDao.observeHistory(userId).map { list ->
    ...
    override suspend fun deleteHistoryItem(id: String) = withContext(Dispatchers.IO) {
        historyDao.deleteById(id)
    }

    override suspend fun clearAllHistory(userId: String?) = withContext(Dispatchers.IO) {
