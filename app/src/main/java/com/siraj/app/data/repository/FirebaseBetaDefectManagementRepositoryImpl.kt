...
class FirebaseBetaDefectManagementRepositoryImpl(
    ...
    override fun getAllDefects(): Flow<List<BetaDefectRecord>> =
        callbackFlow {
    ...
    override fun getDefectById(id: String): Flow<BetaDefectRecord?> = getAllDefects().map { list -> list.find { it.id == id } }
    ...
    override suspend fun triageDefect(
        ...
        try {
            ...
            if (index != -1) {
                ...
                val safeClassification =
                    if (current.domain == DefectDomain.SHARIA_CONTENT &&
                ...
                val safePriority =
                    if (safeClassification == DefectClassification.BLOCKER ||
                        safeClassification == DefectClassification.CRITICAL
                    ) {
        ...
        return try {
            ...
            if (index != -1) {
                ...
                if (newStatus == DefectStatus.RESOLVED &&
                    (current.classification == DefectClassification.BLOCKER || current.classification == DefectClassification.CRITICAL)
                ) {
    ...
    override fun getPrioritizedFixList(): Flow<List<BetaDefectRecord>> =
        getAllDefects().map { list ->
            // الترتيب: P0 Blockers أولاً -> P1 Critical -> P2 Major -> P3 Minor -> Enhancements
            list
                .filter { it.status != DefectStatus.CLOSED && it.status != DefectStatus.DEFERRED }
                .sortedWith(
        ...
        getAllDefects().map { list ->
            DefectTriageSummary(
                ...
                openOrInProgressCount =
                    list.count {
                        it.status == DefectStatus.REPORTED ||
                            it.status == DefectStatus.TRIAGED ||
                            it.status == DefectStatus.IN_PROGRESS
                    },
    ...
    private fun mapDocumentToDefect(
        data: Map<String, Any?>,
        docId: String,
    ): BetaDefectRecord =
        BetaDefectRecord(
