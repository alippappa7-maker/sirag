                            ...
                            try {
                                val profile =
                                    BetaTesterProfile(
                                        ...
                                        group =
                                            snapshot.getString("group")?.let { enumValueOf<TesterGroup>(it) } ?: TesterGroup.COMMUNITY_BETA,
                                        ...
                                        completedJourneys =
                                            (snapshot.get("completedJourneys") as? List<*>)?.mapNotNull { it?.toString() } ?: emptyList(),
    ...
    override suspend fun registerTesterSession(
        ...
        try {
            ...
            CrashMonitoringManager.logBreadcrumb(
                "Tester Session Registered: ${initialProfile.email} (Group: ${initialProfile.group.name}) on ${initialProfile.deviceModel}",
            )
    ...
    override suspend fun submitExperienceSurvey(survey: TesterExperienceSurvey): Result<String> =
        try {
            ...
            if (surveyToSave.testerId.isNotBlank()) {
                testersCollection
                    .document(surveyToSave.testerId)
                    .update("completedSurveysCount", FieldValue.increment(1))
                    .await()
            }

            CrashMonitoringManager.logBreadcrumb(
                "Tester Survey Submitted: Rating ${surveyToSave.overallRating}/5 by ${surveyToSave.testerEmail}",
            )
