...
class AudioRepositoryImpl : AudioRepository {
    ...
    // Mock data in memory
    private val mockTracks = mutableListOf(
        AudioTrack(
            id = "a1",
    ...
    override suspend fun updateProgress(trackId: String, progressSeconds: Int): Resource<Boolean> {
        val index = mockTracks.indexOfFirst { it.id == trackId }
        if (index != -1) {
            val track = mockTracks[index]
    ...
    override suspend fun reportTrack(trackId: String, reason: String): Resource<Boolean> {
        delay(300)
        return Resource.Success(true)
    }
}
