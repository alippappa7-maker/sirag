...
class CostManagementRepositoryImpl(
    ...
    private val providerStatusMap =
        MutableStateFlow<Map<CostProvider, ProviderEmergencyStatus>>(
    ...
    private fun updateMetrics(
        ...
        val alertsMapForWorkspace =
            alertMap.value[workspaceId]?.toMutableMap() ?: mapOf(
    ...
    override fun getWorkspaceUsage(workspaceId: String): Flow<WorkspaceUsageStatus> =
        usageMetricsMap.map { metricsMap ->
            val metrics = metricsMap[workspaceId] ?: UsageMetrics()
            val limits = workspaceLimitsMap[workspaceId] ?: WorkspaceLimits(workspaceId = workspaceId)
            val alerts =
                alertMap.value[workspaceId] ?: mapOf(
                    50 to AlertLevel(50, false),
                    80 to AlertLevel(80, false),
                    100 to AlertLevel(100, false),
                )
    ...
    override fun getProviderEmergencyStatuses(): Flow<List<ProviderEmergencyStatus>> = providerStatusMap.map { it.values.toList() }
