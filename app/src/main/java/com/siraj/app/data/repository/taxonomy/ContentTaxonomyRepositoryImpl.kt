...
class ContentTaxonomyRepositoryImpl : ContentTaxonomyRepository {
    ...
    override fun getClassifiedItems(filter: ContentTaxonomyFilter): Flow<List<ClassifiedContentItem>> {
        return itemsFlow.map { list ->
            list.filter { item ->
                ...
                matchesOrigin && matchesDiscipline && matchesMedia && matchesVerification && matchesRights && matchesQuran && matchesAi && matchesQuery
            }
        }
    }

    override fun getItemById(id: String): Flow<ClassifiedContentItem?> {
    ...
    override suspend fun saveClassifiedItem(item: ClassifiedContentItem): Resource<ClassifiedContentItem> {
        val validatedMeta = ContentTaxonomyEngine.validateAndEnforceTaxonomy(
    ...
    override fun getTaxonomyAuditReport(): Flow<TaxonomyAuditReport> {
        return itemsFlow.map { list -> ContentTaxonomyEngine.auditContentTaxonomy(list) }
    }
