...
class FirebaseContentManagementRepositoryImpl : ContentManagementRepository {
    ...
    private val mockItems = MutableStateFlow(
        listOf(
            AdminContentItem(
                id = "item-1",
                title = "نصيحة الصباح",
    ...
    override fun getManagedContent(filter: ContentManagementFilter, page: Int, limit: Int): Flow<List<AdminContentItem>> {
    ...
    override suspend fun updateContentStatus(contentId: String, newStatus: AdminContentStatus, reason: String?) {
    ...
    override suspend fun updateContentMetadata(contentId: String, metadata: Map<String, Any>) {
        addAuditLog(contentId, "UNKNOWN", "METADATA_UPDATE", "Old Metadata", "New Metadata: $metadata")
    }

    override suspend fun scheduleContent(contentId: String, publishAt: Long) {
        addAuditLog(contentId, "UNKNOWN", "SCHEDULE_PUBLISH", "None", "Publish at: $publishAt")
    }

    override suspend fun getAuditLogs(entityId: String): List<AuditLogEntry> {
        return mockAuditLogs[entityId] ?: emptyList()
    }

    override suspend fun exportAdminReport(): String {
        return "https://siraj.app/reports/admin_report_${System.currentTimeMillis()}.pdf"
    }

    private fun addAuditLog(entityId: String, entityType: String, action: String, oldState: String?, newState: String?) {
