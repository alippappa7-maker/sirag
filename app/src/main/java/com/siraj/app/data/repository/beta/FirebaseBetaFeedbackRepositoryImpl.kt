...
class FirebaseBetaFeedbackRepositoryImpl(
    ...
    override suspend fun submitFeedback(feedback: BetaFeedback): Result<String> {
        return try {
            ...
            val feedbackToSave = feedback.copy(
                id = docId,
            ...
            CrashMonitoringManager.logBreadcrumb("Beta Feedback Submitted: ${feedbackToSave.category.name} - ${feedbackToSave.title} (Severity: ${feedbackToSave.severity.name})")
            if (feedbackToSave.severity == FeedbackSeverity.CRITICAL || feedbackToSave.severity == FeedbackSeverity.HIGH) {
                CrashMonitoringManager.triggerTestNonFatalError("High-Severity Beta Tester Report: [${feedbackToSave.category.name}] ${feedbackToSave.title} - Route: ${feedbackToSave.currentRoute}")
            }
    ...
    override fun getMyFeedback(userId: String): Flow<List<BetaFeedback>> = callbackFlow {
        if (userId.isBlank()) {
            trySend(emptyList())
            close()
            return@callbackFlow
                ...
                val items = snapshot?.documents?.mapNotNull { doc ->
                    try {
                        BetaFeedback(
                            ...
                            category = doc.getString("category")?.let { enumValueOf<FeedbackCategory>(it) } ?: FeedbackCategory.BUG,
                            severity = doc.getString("severity")?.let { enumValueOf<FeedbackSeverity>(it) } ?: FeedbackSeverity.MEDIUM,
