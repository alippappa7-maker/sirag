...
class ReviewerGovernanceRepositoryImpl : ReviewerGovernanceRepository {
    ...
    override fun getReviewers(): Flow<Resource<List<ReviewerProfile>>> {
        return reviewersState.map { Resource.Success(it) }
    }

    override fun getReviewerById(reviewerId: String): Flow<Resource<ReviewerProfile>> {
        return reviewersState.map { list ->
    ...
    override suspend fun suspendReviewer(reviewerId: String, ownerId: String, reason: String): Resource<Unit> {
        val current = reviewersState.value.toMutableList()
        val index = current.indexOfFirst { it.id == reviewerId }
        if (index >= 0) {
            val existing = current[index]
    ...
    override suspend fun reactivateReviewer(reviewerId: String, ownerId: String): Resource<Unit> {
        val current = reviewersState.value.toMutableList()
        val index = current.indexOfFirst { it.id == reviewerId }
        if (index >= 0) {
            val existing = current[index]
    ...
    override suspend fun updateScope(reviewerId: String, scope: ReviewerScope): Resource<Unit> {
        val current = reviewersState.value.toMutableList()
        val index = current.indexOfFirst { it.id == reviewerId }
        if (index >= 0) {
            val existing = current[index]
    ...
    override fun getConflicts(): Flow<Resource<List<ReviewerConflict>>> {
        return conflictsState.map { Resource.Success(it) }
    }
    ...
    override fun getAssignments(): Flow<Resource<List<ReviewerAssignment>>> {
        return assignmentsState.map { Resource.Success(it) }
    }

    override fun getAssignmentsForReviewer(reviewerId: String): Flow<Resource<List<ReviewerAssignment>>> {
        return assignmentsState.map { list ->
            val userAssignments = list.filter {
    ...
    override suspend fun recordDecision(decision: ReviewerDecision): Resource<Unit> {
        ...
        if (aIndex >= 0) {
            ...
            val updatedAssignment = a.copy(
                ...
                status = if (a.isSecondReviewRequired) {
                    if (a.primaryDecisionId != null && (isSecond || a.secondDecisionId != null)) AssignmentStatus.COMPLETED
    ...
    override fun getDecisionsForItem(itemId: String): Flow<Resource<List<ReviewerDecision>>> {
        return decisionsState.map { list ->
