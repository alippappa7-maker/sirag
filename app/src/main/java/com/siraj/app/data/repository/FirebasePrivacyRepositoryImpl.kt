...
class FirebasePrivacyRepositoryImpl(
    ...
    override fun observePrivacyOverview(userId: String): Flow<PrivacyOverviewData> = callbackFlow {
        ...
        val listener = userDocRef.addSnapshotListener { snapshot, error ->
            ...
            deletionReqRef.get().addOnSuccessListener { deletionDoc ->
                val deletionReq = if (deletionDoc != null && deletionDoc.exists()) {
    ...
    private fun calculateLocalPrivacyOverview(userId: String, deletionReq: AccountDeletionRequest?): PrivacyOverviewData {
    ...
    override suspend fun getPrivacyOverview(userId: String): PrivacyOverviewData = withContext(Dispatchers.IO) {
        try {
            var projectsCount = 0
            val projectsSnap = firestore.collection("projects")
                .whereEqualTo("ownerId", userId)
                .get()
                .await()
            projectsCount = projectsSnap.size()

            val deletionDoc = firestore.collection("account_deletion_requests").document(userId).get().await()
            val deletionReq = if (deletionDoc != null && deletionDoc.exists()) {
                AccountDeletionRequest(
                    requestId = deletionDoc.getString("requestId") ?: "",
                    userId = userId,
            ...
            val categories = getStandardRetentionPolicies().map { cat ->
                when (cat.id) {
    ...
    override suspend fun generateUserDataExport(userId: String): Resource<UserDataExportPackage> = withContext(Dispatchers.IO) {
        return@withContext try {
            ...
            val historySnap = firestore.collection("activity_history")
                .whereEqualTo("userId", userId)
                .get()
                .await()
            val historyList = historySnap.documents.mapNotNull { it.data }

            val invoicesSnap = firestore.collection("invoices")
                .whereEqualTo("userId", userId)
                .get()
                .await()
            val anonymizedInvoices = invoicesSnap.documents.mapNotNull { doc ->
    ...
    override suspend fun exportUserDataToJson(userId: String): Resource<String> = withContext(Dispatchers.IO) {
    ...
    override suspend fun saveExportJsonToFile(context: Context, json: String): Resource<File> = withContext(Dispatchers.IO) {
        return@withContext try {
    ...
    override suspend fun clearWatchHistory(userId: String): Resource<Unit> = withContext(Dispatchers.IO) {
        return@withContext try {
            val snap = firestore.collection("activity_history")
                .whereEqualTo("userId", userId)
                .get()
                .await()
    ...
    override suspend fun clearDownloads(userId: String): Resource<Unit> = withContext(Dispatchers.IO) {
        return@withContext try {
            val downloadsDir = File(context.filesDir, "downloads")
    ...
    override suspend fun clearLocalCache(context: Context): Resource<Long> = withContext(Dispatchers.IO) {
        return@withContext try {
    ...
    override suspend fun deleteUserProject(projectId: String): Resource<Unit> = withContext(Dispatchers.IO) {
        return@withContext try {
    ...
    override suspend fun requestAccountDeletion(userId: String, reason: String, gracePeriodDays: Int): Resource<AccountDeletionRequest> = withContext(Dispatchers.IO) {
        return@withContext try {
            ...
            firestore.collection("account_deletion_requests").document(userId).set(map, SetOptions.merge()).await()
    ...
    override suspend fun cancelAccountDeletion(userId: String): Resource<Unit> = withContext(Dispatchers.IO) {
        return@withContext try {
            firestore.collection("account_deletion_requests").document(userId).delete().await()
            firestore.collection("users").document(userId).update(
                mapOf(
    ...
    override suspend fun submitDataCorrection(request: DataCorrectionRequest): Resource<Unit> = withContext(Dispatchers.IO) {
        return@withContext try {
            ...
            firestore.collection("data_correction_requests").document(reqId).set(map).await()
            Resource.Success(Unit)
        } catch (e: Exception) {
            val error = ErrorHandler.handle(e)
    ...
    override fun getStandardRetentionPolicies(): List<StoredDataCategory> {
        return PrivacyManager.getStandardRetentionPolicies()
    }
}
