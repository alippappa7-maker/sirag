...
class FirebaseAiImageGeneratorServiceImpl(
    ...
    override fun generateImage(request: AiImageGenerationRequest): Flow<Resource<List<GeneratedImageItem>>> =
        flow {
            ...
            try {
                ...
                val items =
                    rawImages.map { map ->
                        GeneratedImageItem(
                            id =
                                map["id"] as? String ?: java.util.UUID
                                    .randomUUID()
                                    .toString(),
    ...
    override suspend fun deleteGeneratedImage(
        projectId: String,
        imageId: String,
    ): Resource<Unit> =
        try {
    ...
    override suspend fun attachImageToScene(
        ...
        val targetScene =
            project.scenes.find { it.id == sceneId }
