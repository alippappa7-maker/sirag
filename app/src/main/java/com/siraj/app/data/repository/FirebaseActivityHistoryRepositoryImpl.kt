...
class FirebaseActivityHistoryRepositoryImpl(
    ...
    override suspend fun recordPlaybackPosition(item: UserActivityItem): Unit =
        withContext(Dispatchers.IO) {
    ...
    override suspend fun clearAllHistory(userId: String): Unit =
        withContext(Dispatchers.IO) {
            dao.clearAll(userId)
            try {
                val snapshot =
    ...
    override suspend fun clearCompleted(userId: String): Unit =
        withContext(Dispatchers.IO) {
            dao.clearCompleted(userId)
        }

    override suspend fun clearDownloads(userId: String): Unit =
        withContext(Dispatchers.IO) {
    ...
    override suspend fun syncPending(userId: String): Unit =
        withContext(Dispatchers.IO) {
    ...
    override suspend fun applyRetentionPolicy(userId: String): Unit =
        withContext(Dispatchers.IO) {
            val prefs = observePreferences(userId).firstOrNull() ?: ActivityHistoryPreferences()
    ...
    override suspend fun purgeUserAllData(userId: String): Unit =
        withContext(Dispatchers.IO) {
